...
### PDL-024: SP-3 guidance restructure — Section 6 throughline; draft-first refine workflow; thin synthetic guidance

| Field | Value |
|-------|-------|
| Date | 2026-04-07 |
| Session | SID-20260407-181422 |
| Authored by | User (decision + framing corrections) + Opus (proposal sequence + execution) |

**Context:** Three rounds of upfront figure planning (visuals 3, 4, 5, 7, 8 produced 2026-04-07 morning) produced figures that illustrated imagined categories rather than the prose needs the figures would actually have to serve. The deleted Visual 3 used abstract concept nodes ("Paper body", "Discovery: infrastructure constraints"); Visual 4 mis-staged the v1/v2 contrast; Visual 7 lacked a real timeline; Visual 8 emphasized high-degree but philosophically uninteresting nodes (4.2.11, 4.3.13). All five were deleted. The pattern across the failures: the more elaborate the upfront specification, the worse the fit between figure and argument.

A second pattern: across multiple proposal rounds the assistant kept reaching for the lost-Introduction-chat (4.2.1) and the modlog as the central documentation unit. The user corrected both: (a) the lost chat is one slip in an otherwise working v1/v2 system, not its defining feature; (b) the documentation system was always a *combination* of artifacts, not modlog-centric; (c) the v1/v2 modlog work is the substantive contribution, with CFP-phase tooling as incremental affordance, not regime change.

**Human decisions — four interlocking moves:**

1. **Section 6 as illustrative throughline.** SP-3 follows the history of one section (Section 6) from first writing (2025-10-15, as Section VIII) to current state (2026-04-01, redundancy pass v4.1) across two technological regimes and five production stages. Section 6 is uniquely suited because (i) it carries the meaningful human control integration — the paper's central theoretical move; (ii) §6.3's methodological claim that documentation practice produces its own requirements was empirically generated by the experience of writing Section 6 itself (the SP reconception emerged mid-redraft, 2026-03-02); (iii) its history exercises all three Section 7 criteria (attribution, intellectual trajectory, understanding-and-endorsement) on a single concrete case; (iv) it contains exactly one ghost (the Jan 28 lost draft), which is a clean, scoped limit case for the "where the record falls short" requirement. Reader background must explain what Section 6 is about and why following its history is worth the reader's time before the throughline starts.

2. **Draft-first refine workflow with placeholder figure callouts.** Upfront figure planning has not worked. Pure draft-then-decorate would betray the figures-as-spine principle. The compromise: draft SP-3 prose with concrete `[FIGURE N — purpose, scope, what it must show]` placeholder blocks at each anchor point. Placeholders are specific enough that the prose unfolds around them (introduces, gives reader time, unpacks) but abstract enough that visual details emerge from what the prose actually needs. Each figure is drawn once, against stable surrounding prose, instead of multiple times against guesses. Figures that prove unnecessary when the prose is on the page are dropped without preciousness; figures that become obvious mid-drafting are added.

3. **Thin synthetic guidance replacing the two separated docs.** `CFP_4.4.17_Guidance_SelectedGraphSpecifications.md` (graph specs) and `CFP_4.4.20_SectionGuidance_SP3.md` v6 (story + figure mapping) are merged into one synthetic guidance. The new guidance is thin: story spine + reader-background paragraph + figure callout list with one-line purposes. It does not specify layouts, label text, color choices, or node lists. It carries `supersedes: [CFP_4.4.17, CFP_4.4.20 v6]` in frontmatter.

4. **Figure-detail data lives in a consolidated note, not in guidance.** All layout/label/color/node-list/data-source specifications for the six SP-3 figures live in a single new note (`CFP_5.3.19` or next available number). The note pulls from `CFP_4.7.20_EpistemicTrace_Section6History` for Section 6 data. The drafting AI reads this note when actually drawing each figure; the guidance does not need to carry it.

**The Section 6 trace:** `CFP_4.7.20_EpistemicTrace_Section6History.md` (created 2026-04-07, committed d6457e1) consolidates the full history as the canonical data source for figures and prose alike. UUIDs from Stages 1–2 (Claude.ai web era) are author-confirmed.

**Mapping to Section 7 criteria (six figures, all anchored on Section 6):**

| # | Figure | Section 6 stage | Section 7 criterion |
|---|---|---|---|
| 1 | Section 6 across six months (timeline) | All stages | Trajectory (overview) |
| 2 | The feedback loop (appendix → §6.2) | Stage 2 (Nov 2025) | Trajectory + attribution |
| 3 | Two architectures, one section (diptych) | Stages 1 & 4 | Attribution + understanding-and-endorsement |
| 4 | The three-draft session (version chain) | Stage 4 (Mar 23, 2026) | Trajectory + understanding-and-endorsement |
| 5 | Failure and the visible decision | Stage 3 (Jan 28 – Mar 2, 2026) | Understanding-and-endorsement + acknowledgment of limit |
| 6 | Where Section 6 sits in the project (synthesis) | All stages, in context | All three |

**What is superseded:**
- `CFP_4.4.17_Guidance_SelectedGraphSpecifications.md` — entire file. Last committed state preserved at git d3723f7. The candidate-graph search methodology it specified is preserved in `CFP_5.3.7_SelectedGraphCandidates.md`, which remains valid as a research artifact even though SP-3's figure plan no longer uses its candidates directly.
- `CFP_4.4.20_SectionGuidance_SP3.md` v6 — entire file. Last committed state preserved at git d3723f7. Substantive content (Section 7 criteria, philological methodology, three-phase corpus structure, infrastructure requirements table, CFP chain walk findings, voice and tone, constraints) folds into the new synthetic guidance. The figure plan is replaced by the Section 6 throughline + draft-first workflow.
- PDL-013 (graph-led narrative) is amended, not superseded: the graph-led principle stands; what changes is *how* the figures are produced (draft-first refine instead of upfront specification).

**What carries forward unchanged:** Section 7 criteria as the goals SP-3 must serve; philological methodology; three-phase corpus structure; complementary evidence sources (artifacts and conversations); infrastructure requirements as empirical findings; "different documentation architectures" framing (not weak/strong); honest assessment with error typology; voice and tone constraints.

**Workflow change recorded prospectively:** subsequent SP-3 drafting will proceed by writing prose with placeholder figure callouts. Each figure is drawn one-at-a-time against stable surrounding prose. The drafting AI reads `CFP_4.7.20` (Section 6 trace) and the consolidated figure-data note when generating figures. Refinement happens in context, not in advance.
