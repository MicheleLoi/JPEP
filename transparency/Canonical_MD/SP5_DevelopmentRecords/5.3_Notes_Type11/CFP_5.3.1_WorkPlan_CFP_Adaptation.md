...
## RESUME HERE (last updated 2026-06-14, after the v1.16→v1.20 EthIT-prep cycle — paper renamed `Full_paper_canonical.md`, at v1.20 on `main`)

**Quick state.** Paper: `Paper/MDversion/Full_paper_canonical.md` (**renamed** 2026-06-14 from `CFP_FullPaper_v1.md`; label `CFP_FullPaper_v1` retained as cross-ref ID, `cfp_branch: cfp-ai-ethics-inquiry` added) at **v1.20**. Submitted to *Philosophy & Technology* at v1.10 (14 May 2026); **desk-rejected 2 June 2026** by Floridi (EiC-level "Reject — transfer options available", not a quality verdict). **Next venue: Ethics and Information Technology** (Springer, **double-blind**) — van den Hoven (EiC) co-authored the Santoni de Sio & van den Hoven 2018 paper §6 builds on, so recusal is expected and a Co-Editor handles. Title + abstract held unchanged (author declined the AI-proposed wink-removal, on §3 agent-integrity grounds). Branch `cfp-ai-ethics-inquiry` **merged to `main`** (2026-06-09, `f828fe9`); all work since is on `main`.
