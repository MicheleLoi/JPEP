...
document: "CFP_FullPaper_v1.md — master version history v1 → v1.16 (umbrella modlog)"
project: JPEP
created: 2026-06-10
session_id:
  - SID-20260609-095833
...
# Master Modification Log — CFP_FullPaper_v1.md, v1 → v1.16
...
| **v1.14** | 2026-06-11 | Cut the bias-mitigation Archive apparatus (Liang, Siddiqui/DraftMarks, two bare arXiv IDs — Archive-only, unverified); testimonial post-script shortened to one sentence (~−110 w). BaHammam retained (body-load-bearing). | CFP_4.2.38 (MOD-019) |
| **v1.15** | 2026-06-11 | Orphan reconciliation: re-cited Mecacci & Santoni de Sio (2020) at §6 + Lloyd (2025) at §5.1; removed Boden & Edmonds orphan. Verified the two empirical claims against source PDFs (Abdulhai 68.9% p.10; Schwitzgebel 51%/20%/80% pp.12–15); **caught + fixed a fabricated Abdulhai co-author list**. | CFP_4.2.38 (MOD-020) |
| **v1.16** | 2026-06-11 | Full bibliography verification pass (agent `ad99ac1b`, CrossRef/publisher checks): 33 correct, no new fabrications, all co-author lists confirmed. Fixes: Berg & Robbins year 2024 → 2025; Hosseini/Resnik/Holmes locator added. Reference-reliability closed for submission. | CFP_4.2.38 (MOD-021) |

Two distribution editions are derived from the canonical (not separate versions): `Full_paper_arxiv_v4.md` (public arXiv, author block) and `Full_paper_submission_anon.md` (double-blind EthIT submission, zero identity leak), both regenerated by `derive_distributions.py` after every canonical change.
